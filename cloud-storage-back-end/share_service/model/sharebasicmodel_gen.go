...
var (
	...
	shareBasicRowsExpectAutoSet   = strings.Join(stringx.Remove(shareBasicFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), ",")
	shareBasicRowsWithPlaceHolder = strings.Join(stringx.Remove(shareBasicFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), "=?,") + "=?"
...
func newShareBasicModel(conn sqlx.SqlConn, c cache.CacheConf, opts ...cache.Option) *defaultShareBasicModel {
...
func (m *defaultShareBasicModel) FindOne(ctx context.Context, id uint64) (*ShareBasic, error) {
	shareBasicIdKey := fmt.Sprintf("%s%v", cacheShareBasicIdPrefix, id)
	var resp ShareBasic
	err := m.QueryRowCtx(ctx, &resp, shareBasicIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultShareBasicModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheShareBasicIdPrefix, primary)
}

func (m *defaultShareBasicModel) queryPrimary(ctx context.Context, conn sqlx.SqlConn, v, primary any) error {
	query := fmt.Sprintf("select %s from %s where `id` = ? limit 1", shareBasicRows, m.table)
