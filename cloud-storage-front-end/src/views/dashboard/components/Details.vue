<template>
  <div class="file-details">
    ...
    <Preview :resource="props.resource" style="height: 330px" />
    ...
    <n-modal
      ...
      :mask-closable="false"
    >
      <n-spin v-if="!shareDialog.url" class="share-content" :show="shareDialog.loading">
        <n-form
          ref="formRef"
          :label-width="80"
      ...
      <div v-else class="share-content">
        ...
        <vue-qrcode class="qrcode" :value="shareDialog.url" :options="{ width: 200 }" />
        <div>
          密码：<span>{{ shareDialog.form.password }}</span
          ><br />
          到期时间：<span>{{
            shareDialog.form.deadline == null
              ? "永久"
              : new Date(shareDialog.form.deadline).format("YYYY-MM-DD HH:mm:ss")
          }}</span>
        </div>
      </div>
      <template #action>
